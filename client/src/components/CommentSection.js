...
const CommentSection = ({ symbol }) => {
  ...
  const fetchComments = async () => {
    const res = await fetch(`http://localhost:5000/api/comments/${symbol}`);
  ...
  const handleLike = async (id) => {
    const res = await fetch(`http://localhost:5000/api/comments/${symbol}/${id}/like`, {
      method: "POST",
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(`http://localhost:5000/api/comments/${symbol}/${id}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username }),
    });
