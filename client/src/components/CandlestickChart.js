...
const CandlestickChart = ({ symbol: initialSymbol, isDarkMode }) => {
  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/symbols?q=${inputValue}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const candleRes = await fetch(`http://localhost:5000/api/chart/${symbol}`);
        ...
        const trendRes = await fetch(`http://localhost:5000/api/trendlines/${symbol}`);
        const trendData = await trendRes.json();
        setTrendSegments(trendData.segments || []);
        setBreakpoints(trendData.breaks || []);

        const maRes = await fetch(`http://localhost:5000/api/ma-signals/${symbol}`);
        setMaData(await maRes.json());

        const maMidRes = await fetch(`http://localhost:5000/api/ma-midsignals/${symbol}`);
        setMaMidData(await maMidRes.json());

        const maLongRes = await fetch(`http://localhost:5000/api/ma-signals-50-200/${symbol}`);
        setMaLongData(await maLongRes.json());

        const mostRes = await fetch(`http://localhost:5000/api/most/${symbol}`);
        setMostData(await mostRes.json());

        const macdRes = await fetch(`http://localhost:5000/api/macd/${symbol}`);
  ...
  useEffect(() => {
    ...
    const fetchMostEMA = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/most-ema/${symbol}`);
