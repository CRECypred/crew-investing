...
from most import MOSTScreener  # EMA tabanlı MOSTScreener kullanılacak

# --- 1) Yol tanımları
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
PRICES_DB_PATH = os.path.join(BASE_DIR, "hissedata.db")
SIGNALS_DB_PATH = os.path.join(BASE_DIR, "most_ema_signals.db")  # Yeni db adı

# --- 2) Tabloyu oluştur
def create_most_ema_table():
    conn = sqlite3.connect(SIGNALS_DB_PATH)
    c = conn.cursor()
    c.execute('''
        CREATE TABLE IF NOT EXISTS most_ema_signals (
...
# --- 3) MOST EMA sinyallerini tespit et ve kaydet
def get_latest_most_ema_signals():
    create_most_ema_table()
    ...
    for symbol, df in df_all.groupby("symbol"):
        ...
        try:
            # EMA bazlı MOST için ma_type='EMA'
            screener = MOSTScreener(length=3, stop_loss_percent=2.0, ma_type="EMA")
        ...
        cursor.execute('''
            INSERT OR IGNORE INTO most_ema_signals
...
if __name__ == "__main__":
    sigs = get_latest_most_ema_signals()
    print(f"MOST EMA: {len(sigs)} hisse için son sinyal kaydedildi.")
